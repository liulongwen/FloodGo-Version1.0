    ...
    with torch.no_grad():
        for X, y1 in data_iter:
            ...
            with autocast():  # Semi-precision training
                ...
                l1 = loss1(y1_hat, y1)
                loss = l1
                metric.add(loss * y1.numel(), d2l.accuracy(y1_hat, y1), y1.numel())
...
def train_ch6(MyModule1, train_iter1, test_iter1, num_epochs1, lr1, device, is_real_train=True):
    ...
    # optimizer = torch.optim.NAdam(model_new.parameters(), lr=lr1, weight_decay=1e-4)  # weight_decay=1e-4
    # optimizer = torch.optim.Adagrad(model_new.parameters(), lr=lr1, weight_decay=1e-4)  # weight_decay=1e-4
    ...
    for epoch in range(num_epochs1):
        ...
        for i, (X1, y1) in enumerate(train_iter1):
            ...
            with autocast():
                optimizer.zero_grad()
                y1_hat = model_new(X1)
                policy_loss = loss1(y1_hat, y1)
                loss = policy_loss
                loss.backward()
                optimizer.step()
                with torch.no_grad():
                    metric.add(loss * X1.shape[0], d2l.accuracy(y1_hat, y1), X1.shape[0])
                train_l = metric[0] / metric[2]
                train_acc = metric[1] / metric[2]
        if is_real_train:
            print(f'epoch {epoch + 1}, train loss1 {train_l:.3f}, train acc {train_acc:.3f}')
        else:
        ...
        if is_real_train:
            if (epoch + 1) % 10 == 0:
                ...
                torch.save(model_new.state_dict(), r'current_policy.model')  # Save the parameter file for the current model
                # Compare the winning rate of the current model and the best model, and save the best one
                if train_acc > best_train_acc or abs(train_acc > best_train_acc) < 0.03:
                    print("A better strategy network has emerged!\n")
                    best_train_acc = train_acc
                    torch.save(model_new.state_dict(), r'best_policy.model')  # Save the parameter file of the optimal model
                    number1 = number1 + 1
...
print(f'p={0.3}, weight_decay={0.01}, n_splits={5}, random_state={48}\n')
...
for fold, (train_index, val_index) in enumerate(k_fold.split(X)):
    ...
    # train dataset
    X_train, y_train = generate_dataset_from_sheet_bata(sheet_name_val)
    # validation dataset
    X_val, y_val = generate_dataset_from_sheet(sheet_name_val)
    ...
    val_acc = train_ch6(PNet, train_iter, val_iter, num_epochs, lr, d2l.try_gpu(), is_real_train=False)
...
for i, val_acc1 in enumerate(val_acc_list):
    print(f"The accuracy of Fold {i}: {val_acc1}")
print(f"\nThe average accuracy on the verification set after K-fold cross-validation: {round(val_acc_avg, 3)}\n")
...
policy_labels_1 = np.concatenate(
    (policy_labels1984_1999, policy_labels2000_2023, policy_labels_05_1984, policy_labels_05_2000,
     policy_labels_08_1984), axis=0)

# Connected data matrix
features_2 = np.concatenate((features_08_2000, features_12_1984, features_12_2000, features_15_1984, features_15_2000), axis=0)
policy_labels_2 = np.concatenate(
    (policy_labels_08_2000, policy_labels_12_1984, policy_labels_12_2000, policy_labels_15_1984, policy_labels_15_2000), axis=0)

# assembly dataset
features_train = np.concatenate((features_1, features_2), axis=0)
policy_labels_train = np.concatenate((policy_labels_1, policy_labels_2), axis=0)
features_val = np.array([])
policy_labels_val = np.array([])

# Convert a matrix to a tensor
# train dataset
features_train = torch.tensor(features_train, dtype=torch.float16)
policy_labels_train = torch.tensor(policy_labels_train, dtype=torch.long)
# validation dataset
features_val = torch.tensor(features_val, dtype=torch.float16)
policy_labels_val = torch.tensor(policy_labels_val, dtype=torch.long)

# Load dataset
print("Load a batch dataset")
train_iter = load_array((features_train, policy_labels_train), batch_size, is_train=True)
val_iter = load_array((features_val, policy_labels_val), batch_size, is_train=False)
print("===================================================\n")

val_acc = train_ch6(PNet, train_iter, val_iter, num_epochs, lr, d2l.try_gpu(), is_real_train=True)

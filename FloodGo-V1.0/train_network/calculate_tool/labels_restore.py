...
def labels_restore4_true(labels, device, labels_preprocess=True):
    labels = labels.to(device)
    if labels_preprocess:
        labels_trues = int(labels.item())
        trues = (labels_trues + 1) * 50
    else:
        trues = int(labels.item())
        temp = round(trues / 50) - 1
        if temp == -1:
            temp = 0
        labels_trues = temp
...
def labels_restore4_predict(labels_probs, device):
    labels_probs = labels_probs.to(device)
    labels_probs = labels_probs.to(torch.float32)
    labels_probs = labels_probs.round(decimals=8)
    indices = int(d2l.argmax(labels_probs, axis=1))
    out_flood_data = (indices + 1) * 50
